...
function Testing() {
  ...
  const handleSubmit = async () => {
    const response = await axios.post("https://crudcrud.com/api/dd97adf93f3c4645a2fee7647b5976f0/kocak", {
  ...
  const getData = async () => {
    const response = await axios.get("https://crudcrud.com/api/dd97adf93f3c4645a2fee7647b5976f0/kocak");
    setData(response.data);
    console.log(response.data);
  }
  const deleteHasil = async (id) => {
    const response = await axios.delete(`https://crudcrud.com/api/dd97adf93f3c4645a2fee7647b5976f0/kocak/${id}`);
  ...
  const updateHasil = async (id) => {
    ...
    const response = await axios.put(`https://crudcrud.com/api/dd97adf93f3c4645a2fee7647b5976f0/kocak/${id}`, {
      name: namel,
      age: agel, 
      colour: colourl,
        ...
        <tbody>
          {data.map((l, i) => (
            <tr>
              ...
              <td>{l.age}</td>
              <td>{l.colour}</td>
              <button type="button" onClick={() => deleteHasil(l.id)}>hapus</button>
             <button type="button" onClick={() => updateData(l)}>update</button>
            </tr>
          ))}
        </tbody>
      </div>
